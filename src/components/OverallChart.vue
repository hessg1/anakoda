<template>
  ...
  <div>
    ...
    <v-container v-resize="onResize">
      <h3>Überblick: {{ displayRange[0] }} bis {{ displayRange[1] }}</h3>
...
<script>
  ...
  export default {
      ...
      queryChartData(query){
        if (this.$midataService.isReady()) {
          this.$midataService.getData(query).then(res => {
            ...
            // we can only display valid entries
        ...
        for(let i in data){
          if(!data[i].meta.invalid){
            ...
            if(data[i].category == 'Headache'){
              ...
              headache.push(['Kopfschmerzen', '', tooltip, style, start, end]);
            }
            ...
            if(data[i].category == 'VariousComplaint'){
              ...
              let start = data[i].startTime;
              let end = data[i].endTime;
              compAndCond.push(['Auffälligkeiten', '', tooltip, style, start, end]);
            }
            ...
            if(data[i].category == 'Condition'){
              ...
              let start = data[i].startTime;
              let end = data[i].endTime;
              compAndCond.push([category, '', tooltip, style, start, end]);
            }
            ...
            if(data[i].category == 'SleepPattern'){
              ...
              let start = data[i].startTime;
              let end = data[i].endTime;
              sleep.push(['Schlaf', '', tooltip, style, start, end]);
            }
    ...
    mounted() {
      this.queryChartData('Observation?date=ge' + this.displayRange[0] + '&date=le' + this.displayRange[1]);
    },
